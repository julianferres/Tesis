...
\title{\ttitlefirst} % Defines the thesis title - don't touch this
...
%\begin{titlepage}
	...
	%\author{
        ...
        %\texttt{S.J.Thompson@kent.ac.uk}\\[2.5ex]
...
% TITLE PAGE
%----------------------------------------------------------------------------------------
\def\univname{Universidad de Buenos Aires}
\def\HRule{\rule{\linewidth}{0.8pt}}
\def\ttitlefirst{Verificación de smart contracts en}
\def\ttitlesecond{Marlowe para la blockchain Cardano}
\def\authornames{Julián Ferres\\ \quad}
\def\supname{Dr.\ Ing. Mariano G. Beiró\\Phd. Simon Thompson}

\begin{titlepage}
  \centering
  \begin{tabularx}{\linewidth}{@{}lXr@{}}
      \makecell{\includegraphics[height=3cm]{UBA_Logo.png}}
      & &\makecell{\includegraphics[height=2.5cm]{FIUBA_Logo.png}}\\
  \end{tabularx}\\%
  \vfill
  %
  \textsc{\LARGE \univname}\\[1.5cm] % University name
  \textsc{\large TESIS DE GRADO EN INGENIERÍA INFORMÁTICA}\\[0.5cm] % Thesis type

  \HRule \\[0.667cm] % Horizontal line
  {\huge \bfseries \ttitlefirst}\\[0.4cm] % Thesis title
  {\huge \bfseries \ttitlesecond}\\[0.4cm] % Thesis title
  ...
  \large \textit{Departamento de Computación \\ Facultad de Ingeniería}\\[0.3cm] % University requirement text
  \vfill
  {\large \today}\\[1ex] % Date
  \vfill
...
Al terminar esta tesis, quiero expresar mi gratitud a aquellas personas que tuvieron impacto en la misma:
{ \setlength{\parindent}{24pt}

    \textit{A mi director, Dr.\ Ing. Mariano G. Beiró, por la dedicación y apoyo que ha brindado a este trabajo, por el respeto y confianza a mis sugerencias e ideas y por la guía a lo largo de esta tesis.}

    \textit{A mis profesores, de todos los niveles educativos y competiciones en las que me he involucrado, por la educación impartida y los valores inculcados a lo largo de los años.}

    \textit{Un trabajo de investigación es siempre fruto de ideas, proyectos y esfuerzos previos. En este caso mi más sincero agradecimiento a Pablo Lamela Seijas, Yves Hauser y a todo el equipo de investigación y desarrollo de IOHK, por la oportunidad de contribución y la supervisión durante los aportes de este trabajo.}

    \textit{A los miembros del jurado, por haber aceptado la lectura y evaluación de este trabajo.}
}

También en lo personal quiero agradecer:
{ \setlength{\parindent}{24pt}

    \textit{A mi familia, por el impresionante esmero y sacrificio que han realizado para que tengamos una buena educación, por los valores aprendidos y el gusto a la dedicación y el esfuerzo.}

    \textit{A Ana, quién me ha acompañado en incontables momentos mientras realizaba este trabajo.}

    \textit{A mis amigos y compañeros de la facultad, por entender mis ausencias durante los largos momentos de estudio y desarrollo de este trabajo y por hacer aún más entretenido e interesante el transcurso de la carrera.}

    \textit{Finalmente, reconocer a todas las personas que me han brindado su tiempo, paciencia, comprensión y solidaridad a lo largo de esta tesis y durante mis años de estudio y formación.}
}
...
Cardano~\cite{cardano_docs} es una plataforma blockchain de tipo \textit{proof-of-stake} (prueba de participación, ver~\ref{subs_pos}) descentralizada de tercera generación y el hogar de la criptomoneda \textit{ada}. Es la primera plataforma de cadena de bloques que evoluciona a partir de una filosofía científica y un enfoque impulsado por la investigación.
...
\subsection{\label{subs_pos}Proof of Stake}
...
La plataforma Cardano, así como otras plataformas de criptomonedas como Bitcoin, utilizan en su lugar un enfoque contable conocido como UTXO~\cite{cardano_utxo_ledger} (\textit{Unspent transaction output}, o `Salidas de transacción no utilizadas').
...
Para mantener la integridad de la contabilidad, las nuevas transacciones deben tener todos los outputs no gastados (totalizando el valor de los mismos) utilizados como entrada.
...
Las claves del mapa son índices únicos dentro del contexto del mapa, de manera tal que la combinación del id de la transacción y dicho índice identifica de forma global a dicho output.
...
Hay que tener en cuenta que se debe realizar un cálculo explícito de la cantidad total de \textit{ada} en las salidas y el total de \textit{ada} en todas las entradas de una transacción como parte de la validación de la transacción. También podría haber outputs en una transacción sin inputs correspondientes; estos se deben a la recolección de recompensas.
...
Si bien Marlowe está diseñado para trabajar con cadenas de bloques en general, algunos detalles de cómo interactúa con la cadena son relevantes al describir la semántica y la implementación del lenguaje.
...
Los mecanismos por los cuales se generan estos bloques, y por quién, no son relevantes para el alcance de este documento. Los contratos se expresarán en términos de números de \textit{slot}, contados desde el bloque de inicio (``génesis'') de la cadena de bloques.

Actualmente, se utilizan números de \textit{slot} en la simulación en el \\Marlowe Playground~\cite{marlowe_playground}, pero en la blockchain usaremos el tiempo, adoptando el estándar de tiempo \textit{Posix}.
...
\subsubsection{Pay}
Un contrato de pago \texttt{(Pay acc payee tok val cont)} realizará un pago de valor \texttt{val} de un token \texttt{tok} desde una cuenta \texttt{acc} a un beneficiario \texttt{payee}, quién será uno de los participantes del contrato, u otra cuenta en el mismo.

Se generarán \textit{warnings} si el valor \texttt{val} no es positivo, o si no hay recursos suficientes en \texttt{acc} para realizar el pago en su totalidad (incluso si hay balances positivos de otros tokens en la misma). En este último caso, se realizará un pago parcial (conteniendo todo el dinero disponible). El contrato en el que seguirá la ejecución es \texttt{cont}.
...
\subsubsection{If}
El condicional \texttt{If obs cont1 cont2} continuará en \texttt{cont1} o \texttt{cont2}, dependiendo de la observación \texttt{obs} cuando el mismo es ejecutado.

\subsubsection{When}
Es el constructor de contratos mas complejo, con la forma \texttt{When cases timeout cont}. El mismo es activado por acciones, que pueden o no ocurrir en un \textit{slot} en particular. Como continúa el mismo tras una acción se declara en la sintaxis de \texttt{cases} del contrato.
...
\begin{code}[title=Funciones \textit{agreement} y \textit{arbitrate}.]{Haskell-Cardano}
agreement :: Contract
agreement =
  If
...
\begin{code}[title=Función \textit{price}.]{Haskell-Cardano}
price :: Value
price = Constant 450
\end{code}
...
Además de las teorías que incluye por defecto la distribución de Isabelle/HOL\footnote{\url{https://isabelle.in.tum.de/library/HOL}}, se encuentra disponible un archivo de pruebas formales (\url{https://isa-afp.org}), el cual está en constante crecimiento y en el que cualquier persona puede contribuir.
...
Todos los valores de tipo \texttt{nat} son generados por los constructores \texttt{0} y \texttt{Suc}. Por lo tanto, los valores de tipo nat son \texttt{0}, \texttt{Suc 0}, \texttt{Suc (Suc 0)}, etc. Algunas de las funciones predefinidas son: +, *, $\leq$, etc. A continuación vemos cómo podríamos definir nuestra propia operación de suma:
...
\subsubsection{Minimizando la prueba}
...
\begin{enumerate}
    ...
    \item Es deseable que dos mapas iguales sean representados de la misma forma, entonces se forzó a las claves a estar en orden ascendente.
...
El mismo se ocupa de tomar los términos del contrato de un archivo \texttt{ex\_com1.json} (de sintaxis similar a los de ACTUS) y ejecutar el generador. Luego valida que las transacciones computadas sean las esperadas, así como también que los balances en las wallets correspondientes sean consistentes.
...
El contrato COM tiene una implementación similar al contrato STK (Stock), debido a que comparte alguna de las funciones de cambio de estado. Los detalles de la implementación, junto con la de los demás contratos, se encuentran disponibles en la rama\newline\texttt{fiuba-collaboration-writing-actus-contracts}\footnote{Disponible en el \href{https://github.com/input-output-hk/marlowe-cardano/tree/fiuba-collaboration-writing-actus-contracts}{repositorio marlowe-cardano de input-output-hk}}.
...
El contrato traducido toma tokens de \textit{ada} desde una parte y tokens de dólar desde la otra y los intercambia atómicamente.
...
\subsection{Generación de código Marlowe de algunos contratos COM y PAM}
...
Como se menciono en~\ref{sec:Semantica_marlowe_isabelle}, no se ha desarrollado aún ningún tipo de traductor de Haskell a Isabelle/HOL.\@ Esto obligó a que se debieran traducir manualmente los contratos generados:
...
A continuación mostramos algunas ejecuciones usando \textit{computeTransaction}, mostramos en color rojo la salida producida.
...
También se hizo uso de \textit{Sledgehammer} (detallado en~\ref{ssec:Sledgehammer}) para completar la prueba, quien sugirió utilizar la última linea de la misma.
...
En el caso del contrato PAM, una vez que se incluye el módulo \textit{ValidState} y se pone a un estado válido como precondición de la prueba, se logra la siguiente prueba:
...
\section{Probando la ausencia de \textit{warnings} en el contrato \textit{Auction}}
...
Con respecto a la implementación de IOHK, la misma se encuentra escrita en \\Haskell~\cite{auction_haskell}, y forma parte de los ejemplos del \textit{Playground}:
...
Posee varias funciones auxiliares, y tres funciones que se llaman mutuamente:

\begin{itemize}
    \item \texttt{go}: Maneja el contrato en general, y se ocupa de llenar los \textit{cases} de un constructor \texttt{When} de Marlowe con \textit{deposits} y \textit{choices}. Completa el resto de los parámetros del constructor con la información provista por los términos del contrato.

    \item \texttt{choose}: Recibe un \textit{Party} (que \texttt{go} toma de la lista \texttt{qs}) y retorna un \textit{Case} completado adecuadamente. Dicha \textit{Party} se remueve de la lista de \textit{choices} (\texttt{qs}) y se agrega a la de \textit{deposits} (\texttt{ps}). Para completar los campos del \texttt{Case}, se llama nuevamente a \texttt{go}. Para completar los campos del \texttt{Case}, se llama nuevamente a \texttt{go}.

    \item \texttt{deposit}: Si la oferta más alta hasta el momento (\texttt{m}) es menor al valor del depósito que se está procesando, se actualiza \texttt{m} con el nuevo valor y dueño. También se actualiza si nadie ha ofertado aún (simbolizado con el valor \texttt{Nothing} para \texttt{m}). Se remueve a la \textit{Party} de la lista \texttt{ps} y se retorna el \texttt{Case} correspondiente (que termina de ser completado llamando recursivamente a \texttt{go}).
\end{itemize}

El contrato también tiene algunas funciones auxiliares para poder resolver la finalización. Por ejemplo, \texttt{settle} se ocupa de construir el \texttt{Pay} desde el ganador de la subasta (en el caso de que exista) hacia el dueño (\texttt{owner}) de la misma.

Cabe mencionar que el contrato acumuló algunos \textit{deposits} (de las personas que participaron). Todos los mismos (excepto el del ganador), serán devueltos por un \texttt{Close} eventualmente.
...
Al expandir la \textit{fun} al comando más detallado \textit{function}, estas obligaciones de prueba se vuelven visibles y se pueden analizar o resolver manualmente. La notación de \textit{function} es la siguiente:
...
\begin{enumerate}
    \item La opción \textit{secuential} permite el preprocesamiento con superposición de patrones\footnote{Los mismos definen el comportamiento de la función ante determinadas entradas, y obedecen reglas de \textit{pattern matching}}. Sin esta opción, las ecuaciones ya deben estar disjuntas y completas.
...
La definición consta del constructor \textit{function}, seguido por las firmas de ambas funciones (separadas por \textit{and}) y luego por la definición de todas las funciones, para todos los parámetros de entrada posibles.
...
\begin{itemize}
    ...
    \item A diferencia de Haskell, Isabelle no soporta definiciones anidadas de funciones, por lo que las funciones auxiliares tuvieron que ser extraídas (las variables que obtenía del \textit{scope} se convierten en parámetros de las mismas).
    ...
    \item Para comprimir la notación, los términos del contrato (\textit{owner}, \textit{minBid}, \textit{maxBid} y \textit{deadline}) los encapsulamos en una variable de tipo \textit{AuctionTerms} (que es un \textit{record}, y puede contener varios tipos de datos).
...
Se omitieron las definiciones necesarias para las distintas pruebas, así como también las pruebas de exclusión mutua, completitud y terminación en \textit{function}.

\subsubsection{Contratos generados por la Implementación}

Es posible generar contratos en código Marlowe utilizando la función \textit{auction} de la implementación en Isabelle o la función \textit{contract} de la implementación original en Haskell.

En este caso, mostramos los contratos generados para uno y dos \textit{bidders}:

\begin{code}[title=Llamado a la función \textit{auction} para un solo \textit{bidder}.]{Isabelle}
definition role_own :: Party where
"role_own = Role [10]"

definition role1 :: Party where
"role1 = Role [11]"

value "auction role_own 37 39 [role1] 1057"
\end{code}

Esto generará el contrato con un solo participante \textit{role1}, dueño \textit{role\_own}, oferta mínima 37, oferta máxima 39 y \textit{deadline} 1057. El código de salida es el siguiente:

\begin{code}[title=Contracto \textit{Auction} en Marlowe con un solo \textit{bidder}.]{Haskell-cardano}
When
  [Case (Choice (ChoiceId [1] (Role [11])) [(37, 39)])
    (Contract.Let (ValueId [11]) (ChoiceValue (ChoiceId [1] (Role [11])))
      (When
        [Case
          (Deposit (Role [11]) (Role [11]) (Token [] [])
            (UseValue (ValueId [11])))
          (Pay (Role [11]) (Party (Role [10])) (Token [] [])
            (UseValue (ValueId [11])) Close)]
        1057 Close))]
  1057 Close
\end{code}

Se pueden observar solo dos flujos posibles en el desarrollo de la subasta:

\begin{itemize}
    \item El único participante realiza un \textit{Choice} que se encuentra entre los límites de ofertas y del \textit{deadline}. En tal caso, como es el único participante, dicha elección se termina procesando como depósito y por ser la primera (y única) gana la subasta. En este caso se efectúa el pago al dueño y el contrato termina.
    \item Los casos restantes (se supera el \textit{deadline} o no se hace una oferta válida) implican un \textit{settle} sin ganador (que equivale a un \textit{Close}), por lo que se cierra el contrato sin ganador ni pago.
\end{itemize}


La complejidad del contrato aumenta sustancialmente cuando se agrega un segundo \textit{bidder}, debido a que se tienen que contemplar todos los casos y ordenes de ofertas posibles. Solo agregando un participante \textit{role\_2}:
...
definition role2 :: Party where
"role2 = Role [12]"

value "auction role_own 37 39 [role1, role2] 1057"
\end{code}

Obtenemos el siguiente contrato de Marlowe:

\begin{code}[title=Contracto \textit{Auction} en Marlowe con para dos \textit{bidders}.]{Haskell-cardano}
When
  [Case (Choice (ChoiceId [1] (Role [11])) [(37, 39)])
    (Contract.Let (ValueId [11]) (ChoiceValue (ChoiceId [1] (Role [11])))
      (When
        [Case (Choice (ChoiceId [1] (Role [12])) [(37, 39)])
          (Contract.Let (ValueId [12])
            (ChoiceValue (ChoiceId [1] (Role [12])))
            (When
              [Case
                (Deposit (Role [12]) (Role [12]) (Token [] [])
                  (UseValue (ValueId [12])))
                (When
                  [Case
                    (Deposit (Role [11]) (Role [11]) (Token [] [])
                      (UseValue (ValueId [11])))
                    (Contract.If
                      (ValueGT (UseValue (ValueId [11]))
                        (UseValue (ValueId [12])))
                      (Pay (Role [11]) (Party (Role [10])) (Token [] [])
                        (UseValue (ValueId [11])) Close)
                      (Pay (Role [12]) (Party (Role [10])) (Token [] [])
                        (UseValue (ValueId [12])) Close))]
                  1057
                  (Pay (Role [12]) (Party (Role [10])) (Token [] [])
                    (UseValue (ValueId [12])) Close)),
               Case
        ...
        1057 Close)),
   Case (Choice (ChoiceId [1] (Role [12])) [(37, 39)])
    (Contract.Let (ValueId [12]) (ChoiceValue (ChoiceId [1] (Role [12])))
      (When
        [Case (Choice (ChoiceId [1] (Role [11])) [(37, 39)])
          (Contract.Let (ValueId [11])
            (ChoiceValue (ChoiceId [1] (Role [11])))
            (When
              [Case
                (Deposit (Role [11]) (Role [11]) (Token [] [])
                  (UseValue (ValueId [11])))
...
Como se puede apreciar, la cantidad de constructores en el contrato de Marlowe aumenta significativamente. Un ejemplo claro son los trece constructores \textit{When} que posee el mismo.

Esto es una de las razones por las cuales Marlowe puede utilizarse de forma embebida en Haskell. Haskell posee la expresividad necesaria para comprimir, modularizar y reutilizar componentes repetitivas del código en Marlowe, brindando al usuario un contrato mas legible y significativo.

\subsection{Probando la terminación del contrato}

Dado que HOL es una lógica de funciones totales\footnote{Una función se dice total cuando está definida para todos los valores posibles de entrada.}, la terminación es un requerimiento fundamental. Isabelle intenta probar la terminación automáticamente cuando se realiza la definición. Pero en algunas circunstancias es posible que falle, y la misma tiene que ser probada manualmente.
...
Como se menciona en la sección 4 de~\cite{termination-isabelle}, el método \textit{lexicographic\_order} es el más popular en las pruebas de terminación. Puede probar la terminación de una cierta clase de funciones buscando una combinación lexicográfica adecuada de \textit{medidas de tamaño}. Por supuesto, no todas las funciones tienen un argumento de terminación tan simple. Para ellas, podemos especificar la relación de terminación manualmente.
...
Debemos entonces encontrar una medida que sea decreciente (similar a \textit{evalBound} en~\ref{lst:evalBound}), para cada ejecución de nuestras funciones. Teniendo en cuenta los requerimientos, se utilizaron las siguientes propiedades del contrato para la función \textit{evalBoundAuction}.
...
Teniendo en cuenta la sintaxis de recursión mutua definimos los tipos necesarios y la función \textit{evalBoundAuction}, que respeta las propiedades deseadas.

Como Isabelle transforma nuestra definición triple en una función de tipo \textit{suma de tres funciones}, el mismo será de la forma $\tau_1 + (\tau_2 + \tau_3)$ (para más funciones, los tipos de las nuevas definiciones se anidan a derecha). 
...
goal (8 subgoals):
 1. $\bigwedge$qs p.
       p $\in$ set qs $\Longrightarrow$
       3 +
       4 *
       length (filter (($\neq$) p) qs)
       < 4 * length qs
 2. $\bigwedge$qs p.
       p $\notin$ set qs $\Longrightarrow$
       4 *
       length (filter (($\neq$) p) qs)
       < 5 + 4 * length qs
 3. $\bigwedge$ps p.
...
Todos los subgoals se solucionan observando que la función \textit{filter} cumple con las siguientes propiedades:
...
lemma removePresentElementReducesSize :
    "p $\in$ set ls $\Longrightarrow$ length (filter (($\neq$) p) ls) < length ls"
  by (simp add: length_filter_less)

lemma removeAbsentElementMantainsSize : 
    "p $\notin$ set ls $\Longrightarrow$ length (filter (($\neq$) p) ls) = length ls"
  by (metis (mono_tags, lifting) filter_True)
\end{code}


Para incluir dichos \textit{lemmas} en un probador automático, usamos \textit{using}. Con esto, podemos probar todos los \textit{subgoals}. La prueba de terminación queda completada:
...
\subsection{Modelo de reducciones de Marlowe}\label{ssec:Reducciones_marlowe}

Para poder comprender las pruebas realizadas, es necesario conocer en detalle las diferentes funciones de reducción que son parte de la semántica de Marlowe. Las funciones que mencionamos en esta sección se encuentran traducidas completamente a Marlowe en el archivo \textit{Semantics.thy} de~\cite{marlowe-isabelle-repo}.

En la sección~\ref{ssec:Modelo_de_marlowe} pudimos ver las diferentes acciones que se llevan a cabo al procesar una transacción sobre un contrato. Presentaremos superficialmente las funciones encargadas de llevar a cabo dicho procesamiento.

La función \textit{playTrace} (mediante su función auxiliar \textit{playTraceAux}) procesa una lista de transacciones. Cada transacción se ejecuta mediantes la función \textit{computeTransaction}. La misma se ocupa de procesar una transacción en particular.

Como hemos mencionado, una transacción contiene una lista ordenada de entradas o acciones (llamadas \textit{inputs} en la semántica). Dichos \textit{inputs} se analizan mediante la función \textit{applyAllInputs} (junto con su función recursiva auxiliar \textit{applyAllLoop}). A diferencia de las funciones previas, \textit{applyAllLoop} tiene dos tareas:

\begin{enumerate}
    \item Evalúa el contrato paso a paso hasta que no se puede cambiar más sin procesar ninguna entrada, en este momento se llama \textit{quiescent} (o inactivo). Dicha acción se realiza mediante la función \textit{reduceContractUntilQuiescent}.
    \item Procesa la siguiente entrada (con ayuda de la función \textit{applyInput}).
\end{enumerate}

Este proceso del tipo `\textit{reducir hasta inactivo + aplicar la siguiente entrada}' se realiza con todos los \textit{inputs}. En cada paso, el contacto actual y el estado cambiarán, es posible que se procesen algunas entradas y se realicen pagos.

Para reducir el contrato hasta inactivo se deben realizar sucesivas reducciones. Esto es posible ya que \textit{reduceContractUntilQuiescent} utiliza el ciclo de \textit{reductionLoop} para dicha tarea. La función \textit{reductionLoop} llama en cada paso a \textit{reduceContractStep}, hasta que el mismo es irreducible. Distintos constructores, como \textit{When}, \textit{Pay} o \textit{Close} obedecen distintas reglas de reducción.

Por otro, cada \textit{input} se procesa en \textit{applyInput}, quien se encarga de invocar a \textit{applyCases}. Apply cases se ocupa de recibir una lista de \textit{Cases} (del tipo \textit{Choice}, \textit{Deposit} o \textit{Notify}) y ver si el input (del tipo \textit{IChoice}, \textit{IDeposit} o \textit{INotify}) coincide con su respectivo \textit{Case}. En caso de que haya una coincidencia, se realizan las modificaciones correspondientes (según el caso) al estado del contrato.

El manejo de errores y \textit{warnings} y errores se realiza mediante tipos de datos específicos, que modelan los resultados de cada función. Por ejemplo, para la función \textit{reduceContractStep}, se introducen dos tipos de datos que ayudan a expresar su salida:

\begin{code}[title=Tipos de datos \textit{ReduceWarning} y \textit{ReduceStepResult}.]{Isabelle}
...
O por ejemplo, los siguientes tipos de datos que modelan una transacción, y el resultado de \textit{computeTransaction}:

\begin{code}[title=Tipos de datos relacionados a una transacción en Marlowe.]{Isabelle}
...
A continuación se puede ver un diagrama de llamados entre las funciones de la semántica, en el momento de ejecutar un contrato:

\begin{figure}[H]
	\centering
	\includegraphics[height=0.72\textheight]{Dependencias_semantica_marlowe.png}
    \caption{Llamados entre funciones de la semántica de Marlowe.}\label{fig:Dependencias_semantica}
\end{figure}
...
Actualmente, el único contrato que posee dicha prueba para \textit{computeTransaction} es el contrato \textit{Close}, la misma se puede encontrar en el archivo \texttt{CloseSafe.thy}, en~\cite{marlowe-isabelle-repo}. El objetivo del mismo es probar el siguiente teorema:
...
A lo largo de la prueba, se pueden ver múltiples lemas auxiliares, por ejemplo para las funciones \textit{applyAllInputs}, \textit{reductionLoop}, \textit{reduceContractStep}, entre otras. 
...
Las acciones de los usuarios son imprevisibles y pueden generar \textit{warnings} independientes a la semántica. Algunos ejemplos en este contrato podrían ser: intentar ofertar un valor fuera del rango esperado, que se oferte una cantidad negativa, que en la cuenta no haya dinero suficiente para la oferta, entre otras.

Es por eso que se formularon una serie de precondiciones que deben ser satisfechas para poder asegurar que el contrato no genera \textit{warnings}:
...
Como se mencionó en~\ref{ssec:Modelo_de_marlowe}, al ejecutar una transacción se procesan sucesivamente los \textit{inputs} de la misma, y luego de cada uno se realizan \textit{reductionSteps} hasta que el contrato se encuentra en estado \textit{inactivo} (\textit{quiescent}). Probaremos que al aplicar un \textit{input} no se generan \textit{warnings}. Una vez que se definió el invariante, se procede a verificar la prueba para \textit{applyInput}. 

Para comenzar se utilizará inducción estructural en las funciones mutuamente recursivas. El paso inductivo debe de producir una parte del contrato, ya que en caso contrario no podemos avanzar en las funciones de la semántica, que son las que potencialmente podrían producir \textit{warnings}. Es por eso que la inducción se realizará sobre \textit{contractLoop}, \textit{handleChoose} y \textit{handleDeposit} en forma conjunta.

\begin{code}[title=Ausencia de \textit{warnings} durante \textit{applyInput}.]{Isabelle}
lemma applyInputContractLoopNoWarnings : "invariantHoldsForAuction terms m ps qs curState $\Longrightarrow$ ($\bigwedge$ applyWarn newState cont. applyInput env curState head (contractLoop m ps qs terms) = Applied applyWarn newState cont $\Longrightarrow$ applyWarn = ApplyNoWarning)"

  and applyInputHandleChooseNoWarnings : "invariantHoldsForAuction terms m ps qs curState $\Longrightarrow$ x $\in$ set qs $\Longrightarrow$ ($\bigwedge$ applyWarn newState cont. applyCases env curState head [ handleChoose m ps qs terms x ] = Applied applyWarn newState cont $\Longrightarrow$ applyWarn = ApplyNoWarning)"

 and applyInputHandleDepositNoWarnings : "invariantHoldsForAuction terms m ps qs curState $\Longrightarrow$ x $\in$ set ps $\Longrightarrow$ ($\bigwedge$ applyWarn newState cont. applyCases env curState head [ handleDeposit m ps qs terms x ] = Applied applyWarn newState cont $\Longrightarrow$ applyWarn = ApplyNoWarning)"
...
Luego de realizar las simplificaciones correspondientes y aplicar los casos necesarios, se pueden probar los distintos \textit{subgoals}. Para resolver dicho lema fueron necesarios dos lemas auxiliares, que se repetían en algunos de los \textit{subgoals}.

\begin{code}[title=Lemas auxiliares para la prueba sobre \textit{applyInput}.]{Isabelle}
lemma applyCasesOfMap : 
    "($\bigwedge$ p applyWarn newState cont2. p $\in$ set ps $\Longrightarrow$ applyCases env curState head [f p] =
                Applied applyWarn newState cont2 $\Longrightarrow$ applyWarn = ApplyNoWarning) $\Longrightarrow$ 
...
lemma applyCasesDistributiveAgainstAppend : 
    "($\bigwedge$ applyWarn newState cont . applyCases env curState head l1 =
                 Applied applyWarn newState cont $\Longrightarrow$ applyWarn = ApplyNoWarning) $\Longrightarrow$ 
     ($\bigwedge$ applyWarn newState cont . applyCases env curState head l2 =
                 Applied applyWarn newState cont $\Longrightarrow$ applyWarn = ApplyNoWarning) $\Longrightarrow$ 

     (applyCases env curState head (l1 @ l2) =
                 Applied applyWarn newState cont $\Longrightarrow$ applyWarn = ApplyNoWarning)"
  apply (induction l1)
   apply simp
  apply (elim applyCases.elims)
           apply auto
...
\subsubsection{Ausencia de \textit{warnings} en \textit{reduceContractUntilQuiescent}}

Como se mencionó en la sección~\ref{ssec:Reducciones_marlowe}, entre cada procesamiento de un \textit{input} se realiza una reducción hasta que el contrato esta inactivo (con ayuda de la función \textit{reduceContractUntilQuiescent}).

La función \textit{reduceContractUntilQuiescent} actúa como un \textit{wrapper} de \textit{reductionLoop}, que es la que se ocupa de llamar sucesivamente a \textit{reduceContractStep} hasta que la misma retorne \textit{NotReduced}.

Por otro lado, \textit{reduceContractStep} maneja cada tipo de constructor. La demostración del lema se basa en las siguientes propiedades del contrato \textit{Auction}.

En este caso, la observación se basa en que hay una cantidad reducida de casos en los que \textit{reduceContractStep} devuelva \textit{Reduced}:

\begin{itemize}
    \item Si \textit{ps} y \textit{qs} son la lista vacía, \textit{contractLoop} es equivalente a \texttt{settle m terms} (que eventualmente se reducirá a un \textit{Pay} o \textit{Close}, dependiendo de \texttt{m}).
    \item En caso contrario, la definición del contrato coloca un constructor \textit{When}. El contrato ya estará inactivo a menos que se provoque un \textit{timeout} y nuevamente \texttt{contractLoop m ps qs terms} se reducirá a \texttt{settle m terms}. Además, en este caso, se mantiene el estado luego de una reducción (por definición del contrato \textit{Auction}). 
\end{itemize}

Con los casos posibles detallados, hay que demostrar que \texttt{settle m terms} no produce warnings si satisface el invariante (el posible \textit{Pay} al que se reduce \texttt{settle m terms} es la única fuente posible de \textit{warnings} en este caso).

Además, hay que demostrar que las propiedades que hemos listado son verdaderas (y por lo tanto siempre se llega al \textit{settlement} de alguna forma). Dichas pruebas se incluyen a continuación:

\begin{code}[title=Lemas auxiliares para probar la ausencia de \textit{warnings} en \textit{reductionLoop}.]{Isabelle}
lemma reduceContractStepPay_preservesCont : 
    "reduceContractStep env fixSta (Pay accId payee token val cont) = Reduced wa ef sta cont2 $\Longrightarrow$
     cont2 = cont"
  apply auto
  by (smt (verit, best) ReduceStepResult.inject giveMoney.simps old.prod.case)

lemma auctionSettlementSafe_reduceContractStepEmpty : 
    "invariantHoldsForAuction terms m [] [] fixSta $\Longrightarrow$
     reduceContractStep env fixSta (contractLoop m [] [] terms) = Reduced wa ef sta cont $\Longrightarrow$
     cont = Close"
  apply (auto)
  apply (cases m)
...
lemma contractLoopReducedEqualsSettlement : 
    "ps $\neq$ [] $\vee$ qs $\neq$ [] $\Longrightarrow$
     ...
     (cont = settle m terms $\wedge$ fixSta = sta)"
...
lemma contractLoopReducedSatisfyInvariant :
    "ps $\neq$ [] $\vee$ qs $\neq$ [] $\Longrightarrow$
     invariantHoldsForAuction terms m ps qs fixSta $\Longrightarrow$
     reduceContractStep env fixSta (contractLoop m ps qs terms) = Reduced wa ef sta cont $\Longrightarrow$
...
lemma auctionSettlementSafe_reduceContractStep :
    "invariantHoldsForAuction terms m ps qs fixSta $\Longrightarrow$
     reduceContractStep env fixSta (contractLoop m ps qs terms) = Reduced wa ef sta cont $\Longrightarrow$ 
     cont = Close $\vee$ (cont = settle m terms $\wedge$ invariantHoldsForAuction terms m [] [] sta)"
...
Luego, probamos el caso en el contrato recibe las listas vacías y equivale a \texttt{settle m terms}:

\begin{code}[title=Prueba de ausencia de \textit{warnings} en \textit{reduceContractStep} para el caso \textit{settle}.]{Isabelle}
lemma auctionIsSafe_reduceContractStepEmpty : 
    "invariantHoldsForAuction terms m [] [] fixSta $\Longrightarrow$
...
Para finalmente probarlo en el caso general del \textit{contractLoop}:

\begin{code}[title=Prueba de ausencia de \textit{warnings} en \textit{reduceContractStep} para el caso general.]{Isabelle}
lemma auctionIsSafe_reduceContractStep : 
    "invariantHoldsForAuction terms m ps qs fixSta $\Longrightarrow$
     reduceContractStep env fixSta (contractLoop m ps qs terms) = Reduced wa ef newSta cont $\Longrightarrow$
     wa = ReduceNoWarning"
  apply (cases ps)
   apply (cases qs)
...
Para probar que varias reducciones consecutivas no producen \textit{warnings}, preparamos los sugoals para poder utilizar los lemmas previos utilizados. Una vez probado \textit{reductionLoop}, es sencillo probar \textit{reduceContractUntilQuiescent} debido a que es simplemente un \textit{wrapper}:

\begin{code}[title=Prueba de ausencia de \textit{warnings} en \textit{reductionLoop} y \textit{reduceContractUntilQuiescent}.]{Isabelle}
lemma auctionIsSafe_reductionLoop : 
  "wa = [] $\Longrightarrow$ invariantHoldsForAuction terms m ps qs curState $\Longrightarrow$
  reductionLoop reducedBefore env curState (contractLoop m ps qs terms) wa ef = ContractQuiescent reducedAfter reduceWarns pays newState cont $\Longrightarrow$
  reduceWarns = []"
...
lemma auctionIsSafe_reduceContractUntilQuiescent : 
    "invariantHoldsForAuction terms m ps qs fixSta $\Longrightarrow$
...
Debido a que la escritura de los contratos ACTUS en Haskell sigue en etapa de desarrollo por parte de IOHK, no es factible aún traducir los mismos a Isabelle, debido a que sería necesario realizarlo en la totalidad de la estructura del generador.

Otra limitación aparente surge de la traducción de los contratos completos desde Haskell a Marlowe (y consecuentemente a Isabelle). Debido a la complejidad de algunos contratos ACTUS y a la naturaleza de Marlowe de anidar subcontratos, las traducciones suelen tener decenas (y hasta cientos) de niveles de indentación.

Esto dificulta no solo la lectura, sino la traducción manual a Isabelle de decenas de términos del contrato, haciendo compleja la extracción de propiedades para verificar y las pruebas respectivas.
